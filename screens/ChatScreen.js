...
export default function ChatScreen({ navigation }) {
  ...
  const addNewChat = () => {
    setChats((prev) => [
      { id: Date.now().toString(), title: "แชตใหม่" },
      ...prev,
    ]);
  };
  ...
  return (
    ...
    >
        ...
        {chats.map((chat) => (
          <View key={chat.id} style={styles.sidebarItemRow}>
            <TouchableOpacity
              style={{ flex: 1 }}
              onPress={() => {
                /* TODO: โหลดแชต */ closeItemMenu();
              }}
            >
              <Text numberOfLines={1} style={styles.sidebarItemText}>
                {chat.title}
              </Text>
            </TouchableOpacity>
      ...
      <Animated.View
        style={{ flex: 1, transform: [{ translateX: contentAnim }] }}
      >
        {/* Header */}
        ...
        <ImageBackground
          source={{
            uri: "https://upload.wikimedia.org/wikipedia/commons/3/3c/Dharmachakra_Outline.svg",
          }}
          ...
          >
            ...
            <View style={styles.inputContainer}>
              <TextInput
                ...
                // กดปุ่ม Enter/Return แล้วส่ง (iOS/Android)
                onSubmitEditing={sendMessage}
                returnKeyType="send"
                blurOnSubmit={false} // ส่งแล้วคีย์บอร์ดไม่พับ/ยังโฟกัสช่องเดิม
                // สำหรับเว็บ: Enter = ส่ง, Shift+Enter = (กันไว้เผื่อเปิด multiline) ไม่ส่ง
                onKeyPress={(e) => {
                  if (
                    Platform.OS === "web" &&
